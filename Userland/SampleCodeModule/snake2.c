...
void start_gameTwo() {
  ...
  initializeSnake(&snakeP1, PLAYER_ONE_STARTING_X, PLAYER_ONE_STARTING_Y,
                  WHITE);
  initializeSnake(&snakeP2, PLAYER_TWO_STARTING_X, PLAYER_TWO_STARTING_Y,
                  PURPLE);
  drawRandomFaceTwo();
  ...
  while (!flagWallP1 && !flagSnakeP1 && !flagWallP2 && !flagSnakeP2 &&
         !eaten1 && !eaten2) {
    currentTick = call_ticks();
    gameInputTwo();
    if (currentTick - lastTick >= delay) {
      call_drawRectangle(CARAMEL_BROWN, snakeP1.body[snakeP1.length - 1].x,
                         snakeP1.body[snakeP1.length - 1].y, SQUARE_SIZE,
                         SQUARE_SIZE);
      call_drawRectangle(CARAMEL_BROWN, snakeP2.body[snakeP2.length - 1].x,
                         snakeP2.body[snakeP2.length - 1].y, SQUARE_SIZE,
                         SQUARE_SIZE);
      eaten1 = updateSnake(&snakeP1, mapWidth, mapHeight, &flagWallP1,
                           &flagSnakeP1, 1, faceStartingX2, faceStartingY2);
      redrawSnake(&snakeP1);
      if (!eaten1) {
        eaten2 = updateSnake(&snakeP2, mapWidth, mapHeight, &flagWallP2,
                             &flagSnakeP2, 1, faceStartingX2, faceStartingY2);
        redrawSnake(&snakeP2);
      }
...
void drawRandomFaceTwo() {
  ...
  do {
    ...
    collisionSnakeP1 =
        checkSelfCollision(faceStartingX2, faceStartingY2, &snakeP1);
    collisionSnakeP2 =
        checkSelfCollision(faceStartingX2, faceStartingY2, &snakeP2);
